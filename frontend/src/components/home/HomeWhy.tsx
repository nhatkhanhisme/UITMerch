    ...
    >
      ...
      <div className="relative mx-auto grid max-w-canvas gap-10 pt-16 sm:pt-20 lg:grid-cols-[0.75fr_1.25fr] lg:items-end lg:gap-12 lg:pt-24">
        <div>
          ...
          {/* RESPONSIVE */}
          <h2 className="mt-4 font-fredoka text-[34px] font-bold leading-tight text-black-blue sm:text-5xl lg:text-[68px]">
            Một nơi gom đủ tinh thần UIT
          </h2>
          <p className="mt-6 font-sans text-sm leading-7 text-gray sm:text-base sm:leading-8">
        ...
        <div className="grid gap-4 sm:grid-cols-3 sm:gap-5">
          {reasons.map((reason, index) => (
            <article
              ...
              key={reason.title}
            >
              <span className="font-fredoka text-3xl text-peach sm:text-4xl">
                0{index + 1}
              </span>
              <h3 className="mt-6 font-sans text-lg font-bold leading-7 text-black-blue sm:mt-8 sm:text-xl">
                {reason.title}
              </h3>
              <p className="mt-4 font-sans text-sm leading-7 text-gray">
                {reason.body}
              </p>
            </article>
          ))}
